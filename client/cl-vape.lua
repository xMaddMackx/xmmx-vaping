...
RegisterNetEvent("Vape:StartVaping1")
AddEventHandler("Vape:StartVaping1", function(source)
	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				QBCore.Functions.Notify(Locales.success["use_vape"], 'success')
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				TriggerEvent("Vape:StopVaping", source)
			end
		else
			QBCore.Functions.Notify(Locales.error["in_veh"], 'error')
		end
	else
		QBCore.Functions.Notify(Locales.error["is_dead"], 'error') 
	end
end)

RegisterNetEvent("Vape:StartVaping2")
AddEventHandler("Vape:StartVaping2", function(source)
	local ped = GetPlayerPed(-1)
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
...
RegisterNetEvent("Vape:StartVaping3")
AddEventHandler("Vape:StartVaping3", function(source)
	local ped = GetPlayerPed(-1)
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
...
RegisterNetEvent("Vape:StartVaping4")
AddEventHandler("Vape:StartVaping4", function(source)
	local ped = GetPlayerPed(-1)
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
...
RegisterNetEvent("Vape:StartVaping5")
AddEventHandler("Vape:StartVaping5", function(source)
	local ped = GetPlayerPed(-1)
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
...
RegisterNetEvent("Vape:StartVaping6")
AddEventHandler("Vape:StartVaping6", function(source)
	local ped = GetPlayerPed(-1)
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		QBCore.Functions.Notify(Locales.success["stop_vape"], 'success') 
	end
end)
	...
	if IsPlayerAbleToVape then
		...
		if (DoesEntityExist(ped) and not IsEntityDead(ped)) then
			...
			if VapeFailure == 1 then
				...
				QBCore.Functions.Notify("Your Vape got too hot and exploded!", "error") 
			else
				TaskPlayAnim(ped, ad, anim, 8.00, -8.00, -1, (2 + 16 + 32), 0.00, 0, 0, 0)
				PlaySoundFrontend(-1, "Beep_Red", "DLC_HEIST_HACKING_SNAKE_SOUNDS", 1)
			  		Wait(950)
			...
			if IsControlPressed(0, Config.DragControl) then
			  Wait(Config.ButtonHoldTime)
				if IsControlPressed(0, Config.DragControl) then
					...
					Wait(3000)
					TriggerServerEvent('hud:server:RelieveStress', Config.StressReliefAmt)
				end
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Press ~r~\"E\"~c~ to take a hit.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Press ~r~\"Q\"~c~ to reset to rest postion.")
			end
		end
	  Wait(1)
	end
...
function PlayerIsAbleToVape1()
	...
	local prop_name = "brum_blackredcamo_mag"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end

function PlayerIsAbleToVape2()
	IsPlayerAbleToVape = true
	local ped = GetPlayerPed(-1)
	local ad = "anim@heists@humane_labs@finale@keycards"
	local anim = "ped_a_enter_loop"
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local prop_name = "brum_grey_mag"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end

function PlayerIsAbleToVape3()
	IsPlayerAbleToVape = true
	local ped = GetPlayerPed(-1)
	local ad = "anim@heists@humane_labs@finale@keycards"
	local anim = "ped_a_enter_loop"
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local prop_name = "brum_purple_mag"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end

function PlayerIsAbleToVape4()
	IsPlayerAbleToVape = true
	local ped = GetPlayerPed(-1)
	local ad = "anim@heists@humane_labs@finale@keycards"
	local anim = "ped_a_enter_loop"
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local prop_name = "brum_red_mag"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end

function PlayerIsAbleToVape5()
	IsPlayerAbleToVape = true
	local ped = GetPlayerPed(-1)
	local ad = "anim@heists@humane_labs@finale@keycards"
	local anim = "ped_a_enter_loop"
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local prop_name = "bruwhitepurple_mag"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end

function PlayerIsAbleToVape6()
	IsPlayerAbleToVape = true
	local ped = GetPlayerPed(-1)
	local ad = "anim@heists@humane_labs@finale@keycards"
	local anim = "ped_a_enter_loop"
	...
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local prop_name = "brum_voopoo_drag2"
	VapeMod = CreateObject(GetHashKey(prop_name), x, y, z+0.2,  true,  true, true)
	AttachEntityToEntity(VapeMod, ped, GetPedBoneIndex(ped, 4154), 0.00, 0.0000, 0.055, 0.00, 0.0, 60.0, true, true, false, true, 1, true)
end
